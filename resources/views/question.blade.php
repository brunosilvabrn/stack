    ...
    <script>
        ...
        function like(id) {
            ...
            fetch('{{ route('ask.like') }}', {
                ...
                let numberLike = document.getElementById('number-like-'+id);
                ...
                let classLikeAttr1 = ['bg-transparent', 'hover:bg-green-500', 'hover:text-white', 'text-green-700'];
                let classLikeAttr2 = ['bg-green-500', 'text-white', 'hover:bg-transparent', 'hover:text-green-700'];

                if (data.like === 'add') {
                    classLikeAttr1.forEach(remove);
                    classLikeAttr2.forEach(added);
                    numberLike.textContent = parseInt(numberLike.textContent.trim()) + 1;

                }else if(data.like === 'remove') {
                    classLikeAttr1.forEach(added);
                    classLikeAttr2.forEach(remove);
                    numberLike.textContent = parseInt(numberLike.textContent.trim()) - 1;
                }
        ...
        function responder() {
            ...
            let classAttr1 = ['bg-blue-500', 'hover:bg-blue-700'];
            let classAttr2 = ['bg-red-500', 'hover:bg-red-700'];

            if (box.classList.contains('hidden')) {
                classAttr1.forEach(remove);
                classAttr2.forEach(added);

                box.classList.remove('hidden');
                button.textContent ='Fechar';

            } else {
                classAttr1.forEach(added);
                classAttr2.forEach(remove);
